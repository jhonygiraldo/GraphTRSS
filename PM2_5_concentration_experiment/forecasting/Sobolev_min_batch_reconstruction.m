    ...
    for i=1:length(predicted_snapshots)
        ...
        for jj=1:length(epsilon_set)
            ...
            for j=1:length(alpha_set)
                ...
                error_sobolev_batch_random{jj,j}(ii,i) = mean(abs(x_vector_original-x_vector_reconstructed).^2);
            end
        end
    end
end
...
for i=1:size(error_sobolev_batch_random,1)
    for j=1:size(error_sobolev_batch_random,2)
        mean_errors_sobolev_batch(i,j) = mean(error_sobolev_batch_random{i,j});
    end
end
...
mkdir(results_path);
save([results_path 'error_sobolev_batch_random.mat'],'error_sobolev_batch_random',...
    'RMSE','MAE','MAPE','predicted_snapshots','alpha_set','epsilon_set','best_epsilon','best_alpha');