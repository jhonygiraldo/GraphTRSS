...
x_matrix = Data;
D_inv = diag(G.d)^(-1/2);
G.L = sparse(D_inv*G.L*D_inv);
%%
signals_t = size(x_matrix,2);
%% load results
sob = load('../results_normalized_Laplacian/error_sobolev_batch.mat');
qiu = load('../results_normalized_Laplacian/error_qui_batch.mat');
%%
...
%%
results_path = '../results_loss_normalized/';
mkdir(results_path);
