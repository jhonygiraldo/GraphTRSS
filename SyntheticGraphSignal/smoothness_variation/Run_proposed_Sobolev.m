...
epsilon_set_smoothness = [0.1 0.2 0.5 1 2 5 10]; % smoothness level
best_epsilon = zeros(1,length(epsilon_set_smoothness));
best_alpha = zeros(1,length(epsilon_set_smoothness));
for i_epsilon_smoothness = 1 : length(epsilon_set_smoothness)
    i_epsilon_smoothness
    Temp = Tempall(:,:,i_epsilon_smoothness);
    i_test = 1;
    ...
    best_epsilon(i_epsilon_smoothness) = epsilon_set(Indx_Epsilon);
    best_alpha(i_epsilon_smoothness) = alpha_set(Indx_alpha);
    Error_RMSE(i_test,i_epsilon_smoothness) = min(min(error_sobolev_batch_random));
    ...
    for i_test = 2:test_time
        ...
        param.x0 = 0 * param.y;
        x_recon = solver_BR_TVGS(param);
        Error_RMSE(i_test,i_epsilon_smoothness) = norm(Temp(:) - x_recon(:))/sqrt(N*T); %RMSE       
    end
end

save Error_Sobolev_RMSE Error_RMSE epsilon_set_smoothness best_epsilon best_alpha