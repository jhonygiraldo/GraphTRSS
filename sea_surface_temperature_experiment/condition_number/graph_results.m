...
%figure()
semilogx(epsilon,condition_number_sob,'bs-','LineWidth',line_width,'MarkerSize',marker_size);
hold on;
...
set(gca,'FontName','times','FontSize',font_size,'TickLabelInterpreter','Latex',...
    'XScale','log');
title('c) Sea Surface Temperature, $\beta=1$','Interpreter','Latex');
xticks([1e-4, 1e-2, 1e0, 1e2, 1e4, 1e6]);
set(gcf,'Position',[100,100,width,heigth]);
axis tight;
grid on;
saveas(gcf,['condition_sea.svg']);